...
const ImageGallery = () => {
  ...
  // First filter out 'secret' category images
  const publicImages = images.filter(img => img.category !== 'secret');
  
  // Extract unique categories from public images only
  const categories = ['All', ...new Set(publicImages.map(img => img.category).filter(Boolean))];
  ...
  // Group images by title (for projects with multiple images)
  ...
  // Open lightbox with all images from a group
  ...
  if (loading) return <div className="text-center py-16">Loading images...</div>;
    ...
    <div className="min-h-screen bg-white px-2 sm:px-4 md:px-8 pt-8">
      ...
      <div className="flex justify-center">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8 md:gap-10 mb-10 sm:mb-16 w-full max-w-6xl">
          {Object.entries(groupedImages).map(([title, groupImages]) => (
            // In the grid section, update the div to include a Link
            <Link 
            ...
            >
              <div 
                className="w-full aspect-square mb-3 sm:mb-4 overflow-hidden rounded-sm bg-gray-100 relative"
              >
                <img
