...
const AdminPage = () => {
  ...
  const { images, loading, error: imagesError, fetchImages } = useImages({ category: categoryFilter });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        setSuccessMessage(`${result.images.length} image(s) uploaded successfully!`);
      }
  ...
  const handleEdit = (image) => {
    ...
    setIsEditing(true);
    setPreviewUrls([imageService.getImageDataUrl(image.id, image.category === 'secret' ? image.access_token : null)]);
    window.scrollTo({ top: 0, behavior: 'smooth' });
  };
  ...
  const handleDeleteProject = async (title) => {
    const projectImages = groupedImages[title];
    if (projectImages && projectImages.length > 0) {
      if (window.confirm(`Are you sure you want to delete all ${projectImages.length} images in the "${title}" project?`)) {
        try {
  ...
  const handleBulkDelete = async () => {
    ...
    if (window.confirm(`Are you sure you want to delete ${selectedImages.length} selected images?`)) {
      try {
        ...
        setSuccessMessage(`${selectedImages.length} images deleted successfully!`);
        setSelectedImages([]);
        fetchImages();
      } catch (err) {
  ...
  const handleRegenerateToken = async (id) => {
    if (window.confirm('Are you sure you want to regenerate the access token? This will invalidate any existing shared links.')) {
      try {
        ...
        if (shareLink.includes(`/view-project/${id}?`)) {
          const baseUrl = window.location.origin;
          setShareLink(`${baseUrl}/view-project/${id}?access_token=${result.access_token}`);
        }
      } catch (err) {
  ...
  const handleRevokeAccess = async (id) => {
    if (window.confirm('Are you sure you want to revoke access? Clients will no longer be able to view this project.')) {
      try {
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8 mt-16">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-8">
          ...
          <p className="mt-2 text-sm text-gray-600">Upload and manage images for your website</p>
        </div>
        ...
        <div className="bg-white shadow rounded-lg p-6 mb-8">
          <h2 className="text-xl font-medium text-gray-900 mb-4">
            {isEditing ? (selectedProject ? `Edit Project: ${selectedProject}` : 'Edit Image') : 'Upload New Images'}
          </h2>
          <form onSubmit={handleSubmit}>
                    ...
                    <div className="mt-4">
                      <div className="flex flex-wrap gap-2">
                        {previewUrls.map((url, index) => (
                          <div key={index} className="relative h-20 w-20 overflow-hidden rounded-md group">
                            <img
                              src={url}
                              alt={`Preview ${index + 1}`}
            ...
            <div className="mt-6 flex items-center justify-end space-x-3">
              ...
              >
                {loading ? 'Processing...' : isEditing ? 'Update Image' : 'Upload Image'}
              </button>
            </div>
          </form>
        </div>
        ...
        {shareLink && (
          <div className="mt-4 p-4 bg-gray-50 rounded-md mb-8">
            ...
            <p className="mt-1 text-xs text-gray-500">
              Share this link with your client. They will be able to view the project without needing to log in.
            </p>
          </div>
        )}
        ...
        <div className="bg-white shadow rounded-lg p-6">
            ...
            <div className="flex space-x-4">
              ...
              <div className="flex items-center space-x-2">
                <button
                  ...
                  className={`px-3 py-1 text-xs font-medium rounded-md ${viewMode === 'grouped' ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-700'}`}
                >
                  Group by Title
                </button>
                <button
                  onClick={() => setViewMode('individual')}
                  className={`px-3 py-1 text-xs font-medium rounded-md ${viewMode === 'individual' ? 'bg-indigo-100 text-indigo-700' : 'bg-gray-100 text-gray-700'}`}
                >
                  Individual Images
                </button>
              </div>
          ...
          {loading ? (
            ...
            <p className="text-center py-4 text-gray-500">No images found. Upload some images to get started.</p>
          ) : viewMode === 'grouped' ? (
            // Grouped by title view
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {projectsByTitle.map(([title, groupImages]) => (
                <div key={title} className="border rounded-lg overflow-hidden bg-gray-50">
                  <div className="relative">
                    {/* Category badge */}
                    <div className="absolute top-2 right-2 z-10">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${groupImages[0].category === 'built' ? 'bg-green-100 text-green-800' : groupImages[0].category === 'secret' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'}`}>
                    ...
                    <div className="aspect-w-4 aspect-h-3 w-full">
                      <img
                      src={imageService.getImageDataUrl(groupImages[0].id, groupImages[0].category === 'secret' ? groupImages[0].access_token : null)}
                    ...
                    {groupImages.length > 1 && (
                      <div className="absolute top-2 left-2 bg-white bg-opacity-75 rounded-full w-8 h-8 flex items-center justify-center">
                        <span className="text-xs font-medium">+{groupImages.length - 1}</span>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="p-4">
                    ...
                    {groupImages[0].project_id && (
                      <p className="text-sm text-indigo-600 mt-1">
                        Project: {projects.find(p => p.id === groupImages[0].project_id)?.name || `#${groupImages[0].project_id}`}
                      </p>
                    )}
                    {groupImages[0].description && (
                      <p className="text-sm text-gray-600 mt-2 line-clamp-2">{groupImages[0].description}</p>
                    )}
                    <p className="text-sm text-gray-500 mt-2">{groupImages.length} {groupImages.length === 1 ? 'image' : 'images'}</p>
                    ...
                    {groupImages[0].category === 'secret' && (
                      <div className="mt-2 flex space-x-2">
                        ...
                        <button
                          onClick={() => handleRegenerateToken(groupImages[0].id)}
              ...
              {images.map((image) => (
                <div key={image.id} className="border rounded-lg overflow-hidden bg-gray-50">
                  <div className="relative">
                    ...
                    {/* Category badge */}
                    <div className="absolute top-2 right-2 z-10">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${image.category === 'built' ? 'bg-green-100 text-green-800' : image.category === 'secret' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'}`}>
                        {image.category}
                      </span>
                    </div>
                    ...
                    <div className="aspect-w-4 aspect-h-3 w-full">
                      <img
                        src={imageService.getImageDataUrl(image.id, image.category === 'secret' ? image.access_token : null)}
                        alt={image.title}
                        className="w-full h-48 object-cover"
                      />
                    </div>
                  ...
                  <div className="p-4">
                    ...
                    {image.project_id && (
                      <p className="text-sm text-indigo-600 mt-1">
                        Project: {projects.find(p => p.id === image.project_id)?.name || `#${image.project_id}`}
                      </p>
                    )}
                    {image.description && (
